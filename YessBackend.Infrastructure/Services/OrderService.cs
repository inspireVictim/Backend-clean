...
public class OrderService : IOrderService
{
        ...
        int? userId = null)
    {
        ...
        {
            var product = await _context.PartnerProducts.FirstOrDefaultAsync(p =>
                p.Id == item.ProductId &&
            ...
            {
                totalYessCoinsRequired +=
                    (product.OriginalPrice.Value - product.Price) * item.Quantity;
            }
        ...
        if (userId.HasValue)
        {
            ...
            if (wallet != null && wallet.YescoinBalance < totalYessCoinsRequired)
            {
                throw new InvalidOperationException(
                    $"Недостаточно Yess!Coin. Нужно: {totalYessCoinsRequired}, у вас: {wallet.YescoinBalance}");
            }
        ...
        return new OrderCalculateResponseDto
        {
            ...
            Discount = totalYessCoinsRequired,
    ...
    {
        var idempotencyKey = orderRequest.IdempotencyKey
            ?? GenerateIdempotencyKey(userId, orderRequest.PartnerId, orderRequest.Items);
